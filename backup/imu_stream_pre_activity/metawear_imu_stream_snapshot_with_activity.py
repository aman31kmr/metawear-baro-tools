...
class IMUStreamer:
    ...
    def _open_csv(self):
        ...
        if new_file:
            self.csv_writer.writerow(["utc_iso", "epoch_ms", "sensor", "x", "y", "z"])
        ...
        try:
            ...
            try:
                if self.csv_file is not None:
                    ...
                    self.csv_file = None
                    self.csv_writer = None
                    if self.csv_path and os.path.exists(self.csv_path) and os.path.getsize(self.csv_path) > 64:
                        save_imu_session_plot(self.csv_path)
            finally:
...
def run_webui(mac, *, hci=None, csv_path="imu.csv", port=8000, open_browser=True, enable_activity=False):
    activity_est = None
    if enable_activity:
    ...
    if enable_activity:
        print("Activity panel: GET /activity (heuristic standing/walking/running)", flush=True)
    if open_browser:
        try:
...
def main():
    ...
    parser.add_argument(
        "--activity",
        action="store_true",
        help="With --webui: show standing/walking/running heuristic (~1s updates; see har_imu/)",
    )
